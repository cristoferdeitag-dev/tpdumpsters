...
function generateResponse(message: string, language: string): string {
  ...
  if (msg.includes("overweight") || msg.includes("over weight") || msg.includes("too heavy") || msg.includes("weight limit") || msg.includes("sobrepeso") || msg.includes("peso") || msg.includes("tonelada") || msg.includes("ton limit")) {
    return es
      ? "⚖️ **Tarifa por sobrepeso:** **$199/ton prorrateado**\n\n**Peso incluido por tamaño:**\n• 10 yard: 1 ton\n• 20 yard: 2 tons\n• 30 yard: 3 tons\n\nSi excedes el peso incluido, se cobra $199 por cada tonelada adicional (prorrateado). Los materiales pesados como tierra y concreto tienen peso ilimitado en sus paquetes especiales.\n\n¿Preguntas? **(510) 650-2083**"
      : "⚖️ **Overweight fee:** **$199/ton prorated**\n\n**Included weight by size:**\n• 10 yard: 1 ton\n• 20 yard: 2 tons\n• 30 yard: 3 tons\n\nIf you exceed the included weight, it's $199 per additional ton (prorated). Heavy materials like soil and concrete have unlimited weight in their special packages.\n\nQuestions? **(510) 650-2083**";
  }
