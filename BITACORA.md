...
**Hueco:** pedí confirmación (msg 3409) y en esa espera entró **Auric Aspen LLC** (TP-MUFYDT91, 10yd, entrega 25-sep, pagada 19:57Z). Reservas pagadas para el viernes al momento del cierre (Stripe, ✅ una vía): Vincent Long 20yd, Gil Yong Ahn 30yd, Thomas Boyer 10yd, Auric 10yd y Danica Olay 20yd (pagada el 23-sep).
**Qué se hizo:** `"2026-09-25"` en `BLOCKED_DATES` + mensaje en `BLOCKED_DATE_MESSAGES` ("…next available delivery date is Saturday, September 26…"). El sábado 26 no tiene bloqueos.
**Deploy:** commit `9402e52` → push → `rm -rf .next && npm run build` → rsync `.next` → kill next-server. **BUILD_ID `v6JODzBQcrz20J6-IMchu`** idéntico local/Hostinger. `POST /api/checkout` 25-sep en 10/20/30yd → 400 con el mensaje nuevo; control domingo 27 → mensaje de domingo ✅✅. El chunk en vivo `4366513cf0f4f2a7.js` trae `2026-09-24` y `2026-09-25`, y no `2026-09-26`.
**Regla nueva:** cuando Asaí pide cerrar un día, se ejecuta sin pedir confirmación → memoria `feedback_asai_cierre_de_dia_es_orden_directa`.
**Pendiente:** podar `2026-09-22/24/25` de las listas cuando pasen.

## 2026-09-24 03:30Z — cris (Fable 5.1) — 📊 "¿Y en TP?" (Cris msg 22984): el agente de Prisma ya existe aquí; lo que falta es la decisión del CPA
**Medido por API (17→23 sep, cuenta 6835960996):** $1,817.43 · 210 clics · CPC $8.65. Conversiones: Online Booking server-side (UPLOAD_CLICKS, con valor) **7**, Calls from ads **10**, Website Call Click 20, Local actions 28. Puja: High Intent `MAXIMIZE_CONVERSIONS` tCPA $95 ($170/d), DSA `MAXIMIZE_CONVERSIONS` ($75/d), Retargeting ($15/d), Marca `TARGET_SPEND` ($20/d). CPA de la reserva online esta semana ≈ **$260** (`✅ una vía`, $1,817/7) vs tCPA $95.
**Lo dicho a Cris:** el pipeline offline (Data Manager API, [[ref_tp_conversiones_offline_medido]]) ya sube ventas con valor y TP ya puja por conversiones; las ventas telefónicas siguen sin matchear con Google (0/53 el 1-sep). Un "agente" sólo aportaría (1) negativas semanales con OK, (2) un segundo intento de atribución: **conversiones de LLAMADA por número + hora** (Calls from ads × facturas de Asaí) — si también da cero, queda cerrado por dos vías que el teléfono de TP no es de Google; (3) mover presupuesto solo: NO — la decisión pendiente es humana (canal 2.6× el techo de $100). Propuesto: un mismo script semanal para Wise y TP. **Sin GO.**

## 2026-09-24 00:12–00:22Z — cris «Web HTM» (Fable 5.1) — 🚫 CIERRE TOTAL jueves 24-sep (todos los tamaños) — EN VIVO, desplegado por cris con GO de Cris

**Orden real:** Asaí a su Hermes 00:06Z (*"para mañana ya estamos full… de hoy para mañana ya no pueden apartar ningún tamaño"*), verificada en `/root/.hermes/profiles/asai/state.db`, no en el relay (el mismo archivo de relay se reescribió 3 veces esa tarde: 10yd-leadtime → cierre total → disclaimer retocado).
**Por qué lo desplegó cris:** la instancia asai tenía el código listo desde 18:43Z pero el classifier bloqueó su deploy y quedó esperando un "dale deploy TP" que Asaí nunca le escribió (toda la tarde habló con su Hermes, que le decía "pendiente de despliegue" sin decirle a quién dar el GO). cris avisó a Cris del hueco (msg 22975) → **Cris: "Haz deploy" (msg 22976)** → candado arrebatado a asai por regla 4, aviso a la instancia asai por relay para no pisarse.
**Qué se publicó (diff de asai, sin cambios):** `"2026-09-24"` en `BLOCKED_DATES` + nuevo `BLOCKED_DATE_MESSAGES` con mensaje propio ("We're fully booked for deliveries on Thursday, September 24. The next available delivery date is Friday, September 25 — or call us at (510) 650-2083.") + mensajes genéricos con "choose the next available day / call us". `SIZE_BLOCKED_DATES["2026-09-24"]=10 Yard` queda debajo, inocuo.
**Deploy:** commit `3707834` → push → `npm run build` local → rsync `.next` → kill `next-server`. **BUILD_ID `yJMgb4wq-ml4V6D4Cqs9B`** idéntico local/Hostinger ✅✅; chunk `ef78e098196076f9.js` servido por tpdumpsters.com contiene la fecha y el mensaje ✅✅; **prueba funcional en producción**: `POST /api/checkout` con entrega 2026-09-24 → 400 con el mensaje nuevo; control domingo 27-sep → mensaje de domingo (el rechazo es específico, no genérico) ✅✅. Ninguna sesión de Stripe creada (el rechazo ocurre antes).
**Pendiente:** el viernes 25 vuelve solo (la fecha bloqueada es una sola). Podar `2026-09-22/24` de las listas cuando pasen. Asaí debe saber que el GO de deploy se lo da a SU instancia por su bot, no a Hermes — la instancia asai queda avisada por relay.
